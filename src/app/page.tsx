...
type TabType = "lista" | "cards" | "historico" | "publicar";
...
export default function Home() {
  ...
  const [agendadoOk, setAgendadoOk]     = useState<string | null>(null);
  ...
  const fetchAgendamentos = useCallback(async () => {
    try {
      const r = await fetch("/api/agendamentos?status=todos");
      const d = await r.json();
      if (d.ok) setAgendamentos(d.agendamentos);
    } catch {}
  }, []);

  const agendarPost = async (c: import("@/lib/scraper").Concurso, formato: CardFormato, agendadoPara: string | null, imediato: boolean) => {
    setAgendando(c.id);
    try {
      const { gerarLegenda } = await import("@/lib/legenda");
      ...
      if (data.ok) {
        setAgendadoOk(c.id);
        setTimeout(() => setAgendadoOk(null), 3000);
        fetchAgendamentos();
      } else {
  ...
  useEffect(() => { fetchConcursos(); fetchHistorico(); fetchAgendamentos(); }, [fetchConcursos, fetchHistorico, fetchAgendamentos]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060E20", fontFamily: "'Sora',sans-serif", color: "#fff" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,.05)", padding: "0 20px", overflowX: "auto" }}>
        {(["lista", "cards", "publicar", "historico"] as TabType[]).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            {t === "lista" ? "Lista"
              ...
              : t === "publicar" ? `📸 Publicar (${agendamentos.filter(a => a.status === "pendente").length})`
                ...
                <div key={c.id} style={{
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap" }} className="list-btns">
                      ...
                      <Btn color="#E879F9" onClick={e => {
                        e.stopPropagation();
                        agendarPost(c, "feed", null, true);
                      }} disabled={agendando === c.id || agendadoOk === c.id}>
                        {agendadoOk === c.id ? "✓ Agendado!" : agendando === c.id ? "..." : "📸 Publicar"}
                      </Btn>
                      {(() => {
        ...
        {tab === "publicar" && (
          <div style={{ maxWidth: 900 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
              <div>
                <div style={{ color: "#fff", fontSize: 16, fontWeight: 800 }}>📸 Publicar no Instagram</div>
                <div style={{ color: "rgba(255,255,255,.3)", fontSize: 11, marginTop: 4 }}>
                  O worker verifica a fila a cada 5 minutos. Publicações são espaçadas automaticamente.
                </div>
              </div>
              <button onClick={fetchAgendamentos} style={{
                background: "rgba(0,200,150,.1)", border: "1px solid rgba(0,200,150,.2)",
                color: "#00C896", borderRadius: 9, padding: "8px 14px",
            ...
            {/* Publicar agora — concursos com inscrições abertas */}
            <div style={{ marginBottom: 28 }}>
              <div style={{ color: "rgba(255,255,255,.2)", fontSize: 9, letterSpacing: 1.5, marginBottom: 12, textTransform: "uppercase" }}>
                Publicar agora — Inscrições Abertas
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {concursos.filter(c => c.status === "Inscricoes Abertas").slice(0, 15).map(c => {
                  const jaAgendado = agendamentos.some(a => a.concurso_id === c.id && (a.status === "pendente" || a.status === "publicando"));
                  const jaPublicado = agendamentos.some(a => a.concurso_id === c.id && a.status === "publicado");
                  return (
                    <div key={c.id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 150 }}>
                        <div style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>{c.cargo}</div>
                        <div style={{ color: "#00C896", fontSize: 11, marginTop: 2 }}>{c.orgao} — {c.estado}</div>
                        <div style={{ color: "rgba(255,255,255,.3)", fontSize: 10, marginTop: 1 }}>
                          {c.salario} · {c.vagas}
                          {c.diasRestantes >= 0 && c.diasRestantes <= 7 && (
                            <span style={{ color: "#FF4B4B", fontWeight: 700, marginLeft: 6 }}>⚡ {c.diasRestantes}d</span>
                          )}
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        ...
                        <button
                          onClick={() => agendarPost(c, "feed", null, true)}
                          disabled={jaAgendado || jaPublicado || agendando === c.id}
                          style={{
                            background: jaPublicado ? "rgba(0,200,150,.08)" : jaAgendado ? "rgba(255,184,0,.08)" : "linear-gradient(135deg,#E879F9,#C026D3)",
                            color: jaPublicado ? "#00C896" : jaAgendado ? "#FFB800" : "#fff",
                            border: "none", borderRadius: 9, padding: "7px 14px",
                            fontSize: 11, fontWeight: 700,
                            cursor: (jaAgendado || jaPublicado) ? "not-allowed" : "pointer",
                            opacity: (jaAgendado || jaPublicado) ? .7 : 1,
                          }}
                        >
                          {jaPublicado ? "✓ Publicado" : jaAgendado ? "⏳ Na fila" : agendando === c.id ? "Agendando..." : "📸 Publicar agora"}
                        </button>

                        {/* Agendar para depois */}
                        {!jaAgendado && !jaPublicado && (
                          <div style={{ display: "flex", gap: 4 }}>
                            {["09:00", "12:00", "18:00", "21:00"].map(hora => {
                              const hoje = new Date();
                              const [h, m] = hora.split(":").map(Number);
                              hoje.setHours(h, m, 0, 0);
                              const iso = hoje.toISOString();
                              const passou = hoje < new Date();
                              return (
                                <button
                                  key={hora}
                                  onClick={() => !passou && agendarPost(c, "feed", iso, false)}
                                  disabled={passou}
                                  style={{
                                    background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.1)",
                                    color: passou ? "rgba(255,255,255,.2)" : "rgba(255,255,255,.5)",
                                    borderRadius: 7, padding: "6px 10px",
                                    fontSize: 10, fontWeight: 600,
                                    cursor: passou ? "not-allowed" : "pointer",
                                  }}
                                >
                                  {hora}
                                </button>
            ...
            <div>
              ...
              {agendamentos.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 20px", color: "rgba(255,255,255,.2)" }}>
                  ...
                  <div style={{ fontSize: 11, marginTop: 6 }}>Clique em "Publicar agora" em qualquer concurso acima</div>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {agendamentos.map(ag => {
                    ...
                    return (
                      <div key={ag.id} style={{
                        background: "rgba(255,255,255,.025)", border: `1px solid ${cor}22`,
                        borderRadius: 12, padding: "12px 16px",
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ fontSize: 14 }}>{statusIcon[ag.status]}</span>
                            <span style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>{ag.cargo}</span>
                            <span style={{
                          ...
                          <div style={{ color: "rgba(255,255,255,.4)", fontSize: 11, marginTop: 4 }}>
                            {ag.orgao} — {ag.estado} · Formato: {ag.formato}
                          </div>
                          {ag.agendado_para && (
                            <div style={{ color: "#FFB800", fontSize: 10, marginTop: 2 }}>
                              Agendado para: {new Date(ag.agendado_para).toLocaleString("pt-BR")}
                            </div>
                          )}
                          {ag.publicado_em && (
                            <div style={{ color: "#00C896", fontSize: 10, marginTop: 2 }}>
                              Publicado em: {new Date(ag.publicado_em).toLocaleString("pt-BR")}
                            </div>
                          )}
                          {ag.erro_msg && (
                            <div style={{ color: "#FF4B4B", fontSize: 10, marginTop: 4, fontFamily: "monospace" }}>
                              Erro: {ag.erro_msg}
                            </div>
                          )}
                        </div>
                        {(ag.status === "pendente" || ag.status === "erro") && (
            ...
            <div style={{
              marginTop: 28, background: "rgba(255,184,0,.06)", border: "1px solid rgba(255,184,0,.15)",
              borderRadius: 12, padding: "16px 20px",
            }}>
              <div style={{ color: "#FFB800", fontSize: 12, fontWeight: 700, marginBottom: 6 }}>⚙️ Configuração necessária no Railway</div>
              <div style={{ color: "rgba(255,255,255,.4)", fontSize: 11, lineHeight: 1.7 }}>
                Para publicar automaticamente, configure as variáveis de ambiente no Railway:<br/>
                <code style={{ color: "#00C896" }}>IG_USERNAME</code> — seu usuário do Instagram<br/>
                <code style={{ color: "#00C896" }}>IG_PASSWORD</code> — sua senha do Instagram<br/>
                <code style={{ color: "#00C896" }}>WORKER_SECRET</code> — uma senha aleatória para proteger o worker<br/>
                <code style={{ color: "#00C896" }}>NEXT_PUBLIC_APP_URL</code> — URL do seu app no Railway
              </div>
            </div>
          </div>
        )}
